...
CSV_VAL = "data_validation_phi75_theta145_art_def_butter_fc15.csv"


TRAIN_DIR = "/home/jesus/TFM_Baxter/work/ENTRENAMIENTO_DEFINITIVO/FRICCION_SPR_J1J2_DEF/MUÑECA_DESACOP"

THETA_FULL_FILE = os.path.join(TRAIN_DIR, "theta_EXT_full.npy")
SIGMA_FILE      = os.path.join(TRAIN_DIR, "w_sigma_EXT_full.npy")

THETA_J567_FILE = os.path.join(TRAIN_DIR, "theta_EXT_BLOCK_J5J6J7.npy")
THETA_J67_FILE  = os.path.join(TRAIN_DIR, "theta_EXT_BLOCK_J6J7.npy")
THETA_J7_FILE   = os.path.join(TRAIN_DIR, "theta_EXT_BLOCK_J7.npy")

OUT_DIR_VAL = "/home/jesus/TFM_Baxter/work/VALIDACION_DEFINITIVA_EXT"
...
SPRING_JOINTS = [0, 1]       # J1 y J2
P_SPR = len(SPRING_JOINTS)

P_TOTAL = P_BASE + P_OFF + P_SPR  # 93
...
# 2) CARGA VALIDACIÓN
# ===============================
print(f"Leyendo VALIDACIÓN: {CSV_VAL}")
...
N = q.shape[0]
print(f"Muestras VAL: {N}")

# ===============================
# 3) CONSTRUIR matObs_val y tau_vec_val
# ===============================
print(f"Parámetros modelo: base={P_BASE}, offsets={P_OFF}, muelles={P_SPR} -> TOTAL={P_TOTAL}")
...
print("matObs_val:", matObs.shape, "tau_vec_val:", tau_vec.shape)

# ===============================
# 4) CARGA thetas y sigma (TRAIN)
# ===============================
theta_full = np.load(THETA_FULL_FILE)
theta_J567 = np.load(THETA_J567_FILE)
theta_J67  = np.load(THETA_J67_FILE)
theta_J7   = np.load(THETA_J7_FILE)
...
assert sigma.shape[0] == NUM_JOINTS

print("sigma (TRAIN):", sigma.round(6))
print("EPS_VEL (VAL):", EPS_VEL)

# ===============================
# 5) EVALUACIÓN GLOBAL (FULL)
# ===============================
def eval_theta(theta_vec, tag, do_all_joints=True):
    tau_est_vec = matObs @ theta_vec
    tau_est = tau_est_vec.reshape(N, NUM_JOINTS)

    if do_all_joints:
        joints_to_show = list(range(NUM_JOINTS))
    else:
        joints_to_show = [4, 5, 6]

    print(f"\n=== Métricas VALIDACIÓN — {tag} ===")
    ZOOM_LEN = min(2000, N)
    ...
    for j in joints_to_show:
        ...
        plt.plot(tau_hat_j, "--", label=f"τ̂ {tag} J{j+1}")
        plt.title(f"VALIDACIÓN — {tag} Joint {j+1}")
        plt.xlabel("muestra")
        ...
        for (start, end, tag_phase) in [(start1,end1,"PHASE1_RANDOM"), (start2,end2,"PHASE2_SPIRAL")]:
            ...
            axs[2].plot(x_z, tau_est_z, "--", label=f"τ̂ {tag}")
            axs[2].set_ylabel(f"τ{j+1} [Nm]"); axs[2].set_xlabel("muestra")
            axs[2].grid(True); axs[2].legend()
            fig.suptitle(f"VALIDACIÓN — {tag} Joint {j+1} ({tag_phase} ZOOM)")
            plt.tight_layout()
            plt.savefig(os.path.join(OUT_DIR_VAL, f"VAL_{tag}_GLOBAL_J{j+1}_ZOOM_{tag_phase}.png"), dpi=120)
            plt.close(fig)

    return tau_est

tau_est_full = eval_theta(theta_full, "FULL_EXT", do_all_joints=True)

# ===============================
# 6) DIAGNÓSTICO WLS (con sigma de TRAIN) para FULL
# ===============================
w = np.zeros(N*NUM_JOINTS)
for j in range(NUM_JOINTS):
    idx = np.arange(j, N*NUM_JOINTS, NUM_JOINTS)
    w[idx] = 1.0 / (sigma[j] + EPS)
Wsqrt = np.sqrt(w)
A_val_w = matObs * Wsqrt[:, None]
b_val_w = tau_vec * Wsqrt
mse_w_global = float(np.mean((A_val_w @ theta_full - b_val_w)**2))
print(f"\nMSE ponderado global (WLS, diagnóstico, FULL): {mse_w_global:.6f}")

print("MSE ponderado por joint (WLS, diagnóstico, FULL):")
for j in range(NUM_JOINTS):
    err_j = (tau_est_full[:, j] - tau[:, j]) / (sigma[j] + EPS)
    mse_w_j = float(np.mean(err_j**2))
    print(f"  J{j+1}: {mse_w_j:.6f}")

# ===============================
# 7) MODELO REDUCIDO J1 (VALIDACIÓN) — igual que train
# ===============================
print("\n=========== MODELO REDUCIDO J1 (VALIDACIÓN) ===========")

rows_J1 = row_idx_for_joint(0, N, NUM_JOINTS)
YJ1 = matObs[rows_J1, :P_BASE][:, 0:12]  # bloque base de J1
...
rmse_red = float(np.sqrt(mean_squared_error(tau1, tau1_red)))
r2_red = float(r2_score(tau1, tau1_red))
print(f"J1 reducido (VAL): RMSE={rmse_red:.4f} Nm, R2={r2_red:.4f}")

plt.figure(figsize=(10,5))
plt.plot(tau1, label="τ1 real")
plt.plot(tau1_red, "--", label="τ̂1 reducido (VAL)")
plt.title("VALIDACIÓN — REDUCED J1 (τ vs τ̂)")
plt.xlabel("muestra"); plt.ylabel("Torque [Nm]")
plt.grid(True); plt.legend(); plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR_VAL, "VAL_REDUCED_J1_EXT.png"), dpi=140)
plt.close()
...
for (start, end, tag_phase) in [(start1,end1,"PHASE1_RANDOM"), (start2,end2,"PHASE2_SPIRAL")]:
    ...
    axs[2].grid(True); axs[2].legend()
    fig.suptitle(f"VALIDACIÓN — REDUCED J1 ({tag_phase} ZOOM)")
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR_VAL, f"VAL_REDUCED_J1_EXT_ZOOM_{tag_phase}.png"), dpi=130)
    plt.close(fig)

# ===============================
# 8) VALIDACIÓN MUÑECA: comparar modelos desacoplados (J5–J7)
# ===============================
# Aquí NO borramos nada: añadimos evaluación específica de muñeca
eval_theta(theta_J567, "BLOCK_J5J6J7_EXT", do_all_joints=False)
eval_theta(theta_J67,  "BLOCK_J6J7_EXT",   do_all_joints=False)
eval_theta(theta_J7,   "BLOCK_J7_EXT",     do_all_joints=False)

print(f"\n[OK] Resultados de VALIDACIÓN guardados en: {OUT_DIR_VAL}")
